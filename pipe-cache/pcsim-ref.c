...
void do_fetch_stage()
{
    ...
    imem_error = !get_byte_val_I(mem, valp, &instr);
    ...
    instr_valid = ((decode_input->icode) == (I_NOP) || (decode_input->icode) ==  (I_HALT) || (decode_input->icode) == (I_RRMOVQ) || (decode_input->icode)  == (I_IRMOVQ) || (decode_input->icode) == (I_RMMOVQ) ||  (decode_input->icode) == (I_MRMOVQ) || (decode_input->icode) == (I_ALU)   || (decode_input->icode) == (I_JMP) || (decode_input->icode) == (I_CALL) || (decode_input->icode) == (I_RET) || (decode_input->icode) ==  (I_PUSHQ) || (decode_input->icode) == (I_POPQ) || (decode_input->icode) == (I_LEAQ)  || (execute_output->icode) == (I_VECADD) || (execute_output->icode) == (I_SHF));
    ...
    valp++;
    if ((decode_input->icode) == (I_RRMOVQ) || (decode_input->icode) == (I_ALU) || (decode_input->icode) == (I_PUSHQ) || (decode_input->icode)== (I_POPQ) || (decode_input->icode) == (I_IRMOVQ) || (decode_input->icode) == (I_RMMOVQ) || (decode_input->icode) == (I_MRMOVQ) || (decode_input->icode) == (I_LEAQ) || (execute_output->icode) == (I_VECADD) || (execute_output->icode) == (I_SHF)) {
	    get_byte_val_I(mem, valp, &byte1);
	    valp ++;
    }
...
{
    //set srcA
    if(decode_output -> icode == I_RRMOVQ || decode_output -> icode == I_RMMOVQ || decode_output -> icode == I_ALU || decode_output -> icode == I_PUSHQ || execute_output->icode == I_VECADD || execute_output->icode == I_SHF) {
    ...
    //set srcB
    if(decode_output -> icode == I_ALU || decode_output -> icode == I_RMMOVQ || decode_output -> icode == I_MRMOVQ || decode_input->icode == I_LEAQ  || execute_output->icode == I_VECADD || execute_output->icode == I_SHF) {
    ...
    //set destE
    if(decode_output -> icode == I_RRMOVQ || decode_output -> icode == I_IRMOVQ || decode_output -> icode == I_ALU  || execute_output->icode == I_VECADD || execute_output->icode == I_SHF) {
...
void do_memory_stage()
{
    ...
    if (mem_read) {
        if ((dmem_status = get_word_val_D(mem, mem_addr, &valm)) != READY) {
    ...
    writeback_input->status = ((dmem_status == ERROR) ? (STAT_ADR) : (memory_output->status));
...
void do_stall_check()
{
    ...
    execute_state->op = pipe_cntl("EX", e_stall(), e_bubble());
    memory_state->op = pipe_cntl("MEM", m_stall(), m_bubble());
    writeback_state->op = pipe_cntl("WB", w_stall(), w_bubble());
...
long long f_stall() {
   return ((((execute_output->icode) == (I_MRMOVQ) || (execute_output->icode) ==
        ...
        (decode_output->icode) || (I_RET) == (execute_output->icode) || (I_RET)
         == (memory_output->icode))) || (dmem_status == IN_FLIGHT);
}

long long d_stall() {
     return (((execute_output->icode) == (I_MRMOVQ) || (execute_output->icode) ==
        (I_POPQ)) & ((execute_output->destm) == (execute_input->srca) ||
        (execute_output->destm) == (execute_input->srcb))) || (dmem_status == IN_FLIGHT);
}
...
void help() {
    ...
    printf("cycle n           -  execute n cycles\n");
