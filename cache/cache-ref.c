...
void get_word_cache(cache_t *cache, uword_t addr, word_t *dest) {
    ...
    /* your implementation */
    byte_t *dest_b = (byte_t*)dest;
    for (size_t i = 0; i < sizeof(word_t); i++) {
        get_byte_cache(cache, addr + i, dest_b + i);
    }
}
...
{
    byte_t *dest_b = (byte_t*)&val;
    for (size_t i = 0; i < sizeof(word_t); i++) {
        set_byte_cache(cache, addr + i, dest_b[i]);
    }
}
